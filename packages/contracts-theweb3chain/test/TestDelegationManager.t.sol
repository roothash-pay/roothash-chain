...
contract CpDelegationManagerTest is Test {
    ...
    function testStakerCanUndelegate() public {
        ...
        assertEq(theweb3ChainDepositManager.getDeposits(user1), 0);
    ...
    function testOwnerCanSetWithdrawalDelayBlocks() public {
        ...
        vm.prank(owner);
        delegationManager.settheweb3ChainBaseWithdrawalDelayBlocks(newBlocks);
        assertEq(delegationManager.chainBaseWithdrawalDelayBlock(), newBlocks);

        vm.expectRevert();
        vm.prank(user1);
        delegationManager.settheweb3ChainBaseWithdrawalDelayBlocks(456);

        vm.expectRevert(
            "DelegationManager._settheweb3ChainBaseWithdrawalDelayBlocks: _withdrawalDelayBlocks cannot be > MAX_WITHDRAWAL_DELAY_BLOCKS"
        );
        vm.prank(owner);
        delegationManager.settheweb3ChainBaseWithdrawalDelayBlocks(300000);
    }
    ...
    function testOperatorSharesIncreaseViaDelegate() public {
        _registerAndDelegate();
        assertGt(delegationManager.getOperatorShares(operator), 0); // 你可以 mock theweb3ChainDepositManager 返回值
    }
