...
import "../../interfaces/Itheweb3ChainBase.sol";
...
contract RewardManager is RewardManagerStorage, Pausable {
    ...
    function initialize(
        ...
        _initializeRewardManagerStorage(_delegationManager, _theweb3ChainDepositManager);
    }

    function payFee(address chainBase, address operator, uint256 baseFee) external onlyPayFeeManager {
        uint256 totalShares = Itheweb3ChainBase(chainBase).totalShares();
    ...
    function _updateStakerReward(address chainBase, uint256 stakeFee, uint256 totalShares) internal {
        uint256 length = Itheweb3ChainBase(chainBase).stakerListLength();

        for (uint256 i = 0; i < length; i++) {
            address staker = Itheweb3ChainBase(chainBase).stakerListFind(i);
            uint256 shares = theweb3ChainDepositManager.getDeposits(Itheweb3ChainBase(chainBase).stakerListFind(i));
