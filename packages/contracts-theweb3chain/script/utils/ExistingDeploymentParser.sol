...
contract ExistingDeploymentParser is Script, Test {
    ...
    function _parseDeployedContracts(string memory existingDeploymentInfoPath) internal virtual {
        ...
        theweb3ChainLayerProxyAdmin =
            ProxyAdmin(stdJson.readAddress(existingDeploymentData, ".addresses.theweb3ChainLayerProxyAdmin"));
        theweb3ChainLayerPauserReg =
            PauserRegistry(stdJson.readAddress(existingDeploymentData, ".addresses.theweb3ChainLayerPauserReg"));
        ...
        theweb3ChainDepositManager = theweb3ChainDepositManager(
            payable(stdJson.readAddress(existingDeploymentData, ".addresses.theweb3ChainDepositManager"))
        );
        theweb3ChainDepositManagerImplementation = theweb3ChainDepositManager(
            payable(stdJson.readAddress(existingDeploymentData, ".addresses.theweb3ChainDepositManagerImplementation"))
        );

        theweb3ChainBase =
            theweb3ChainBase(payable(stdJson.readAddress(existingDeploymentData, ".addresses.theweb3ChainBase")));
        theweb3ChainBaseImplementation = theweb3ChainBase(
            payable(stdJson.readAddress(existingDeploymentData, ".addresses.theweb3ChainBaseImplementation"))
        );
    ...
    function _parseInitialDeploymentParams(string memory initialDeploymentParamsPath) internal virtual {
        ...
        theweb3ChainBASE_MIN_DEPOSIT = stdJson.readUint(initialDeploymentData, ".theweb3ChainBase.MIN_DEPOSIT");
        theweb3ChainBASE_MAX_DEPOSIT = stdJson.readUint(initialDeploymentData, ".theweb3ChainBase.MAX_DEPOSIT");

        // Read initialize params for upgradeable contracts
        theweb3ChainDEPOSITMANAGER_MANAGER_INIT_PAUSED_STATUS =
            stdJson.readUint(initialDeploymentData, ".theweb3ChainDepositManager.init_paused_status");
    ...
    function _verifyContractsInitialized() internal virtual {
        ...
        delegationManager.initialize(
            address(0), theweb3ChainLayerPauserReg, 0, 0, theweb3ChainDepositManager, theweb3ChainBase, slashingManager
        );

        // theweb3ChainDepositManager
        vm.expectRevert(abi.encodeWithSignature("InvalidInitialization()"));
        theweb3ChainDepositManager.initialize(address(0), delegationManager, theweb3ChainBase);

        // ChainBases
        vm.expectRevert(abi.encodeWithSignature("InvalidInitialization()"));
        theweb3ChainBase(address(theweb3ChainBase)).initialize(
            theweb3ChainLayerPauserReg, 0, 0, theweb3ChainDepositManager
        );
    }
    ...
    function _verifyInitializationParams() internal view virtual {
        ...
        require(theweb3ChainBase.paused() == 0, "theweb3ChainBase: init paused status set incorrectly");

        // Pausing Permissions
        require(
            theweb3ChainLayerPauserReg.isPauser(operationsMultisig), "pauserRegistry: operationsMultisig is not pauser"
        );
        require(theweb3ChainLayerPauserReg.isPauser(executorMultisig), "pauserRegistry: executorMultisig is not pauser");
        require(theweb3ChainLayerPauserReg.isPauser(pauserMultisig), "pauserRegistry: pauserMultisig is not pauser");
        require(theweb3ChainLayerPauserReg.unpauser() == executorMultisig, "pauserRegistry: unpauser not set correctly");
    }
    ...
    function logAndOutputContractAddresses(string memory outputPath) public {
        ...
        vm.serializeAddress(deployed_addresses, "theweb3ChainLayerProxyAdmin", address(theweb3ChainLayerProxyAdmin));
        vm.serializeAddress(deployed_addresses, "theweb3ChainLayerPauserReg", address(theweb3ChainLayerPauserReg));
        vm.serializeAddress(deployed_addresses, "theweb3ChainBase", address(theweb3ChainBase));
        vm.serializeAddress(
            deployed_addresses, "theweb3ChainBaseImplementation", address(theweb3ChainBaseImplementation)
        );
        ...
        vm.serializeAddress(deployed_addresses, "theweb3ChainDepositManager", address(theweb3ChainDepositManager));
        vm.serializeAddress(
            deployed_addresses,
