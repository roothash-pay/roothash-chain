...
contract DelegationManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, Pausable, DelegationManagerStorage {
    ...
    ) DelegationManagerStorage(_cpChainDepositManager, _cpChainBase, _slashingManager) {
    ...
    *******************************************************************************/
    ...
    function slashingStakingShares(address staker, uint256 shares) external onlySlashingManager {
        cpChainDepositManager.removeShares(staker, shares);

        cpChainBase.withdraw(address(slashingManager), shares);
    }
    ...
    function delegateToBySignature(address staker, address operator, SignatureWithExpiry memory stakerSignatureAndExpiry, SignatureWithExpiry memory approverSignatureAndExpiry, bytes32 approverSalt) external {
        require(
    ...
    function increaseDelegatedShares(address staker, uint256 shares) external onlyStrategyManager {
    ...
    function decreaseDelegatedShares(address staker, uint256 shares) external onlyStrategyManager {
        if (isDelegated(staker)) {
            address operator = delegatedTo[staker];
    ...
    *******************************************************************************/
    ...
    function _removeSharesAndQueueWithdrawal(address staker, address operator, address withdrawer, uint256 shares) internal returns (bytes32) {
    ...
    function getStakerSharesOfOperator(address operator) external view returns (address[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < stakerLen; i++) {
            ...
            shares[i] = stakerDelegateSharesToOperator[stakerList[i]][operator];
        }
    ...
    *******************************************************************************/
    ...
    function calculateDelegationApprovalDigestHash(address staker, address operator, address _delegationApprover, bytes32 approverSalt, uint256 expiry) public view returns (bytes32) {
