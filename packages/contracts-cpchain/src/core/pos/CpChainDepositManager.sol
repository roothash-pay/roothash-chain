...
contract CpChainDepositManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, CpChainDepositManagerStorage {
    ...
    constructor(IDelegationManager _delegation, ICpChainBase _cpChainBase) CpChainDepositManagerStorage(_delegation, _cpChainBase) {
    ...
    function depositIntoCpChain(uint256 amount) external payable nonReentrant returns (uint256 shares)  {
        shares = _depositIntoCpChain(msg.sender, amount);
    }

    function depositIntoCpChainWithSignature(uint256 amount, address staker, uint256 expiry, bytes memory signature) external payable nonReentrant returns (uint256 shares) {
    ...
    function removeShares(address staker, uint256 shareAmount) external onlyDelegationManager {
        require(shareAmount != 0, "CpChainDepositManager.removeShares: shareAmount should not be zero!");
