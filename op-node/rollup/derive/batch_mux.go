...
func (b *BatchMux) Reset(ctx context.Context, sysCfg eth.SystemConfig) error {
	// TODO(12490): change to a switch over b.cfg.ActiveFork(base.Time)
	switch {
	default:
		if _, ok := b.SingularBatchProvider.(*BatchQueue); !ok {
			b.log.Info("BatchMux: activating pre-Holocene stage during reset")
			b.SingularBatchProvider = NewBatchQueue(b.log, b.cfg, b.prev, b.l2)
		}
	}
	return b.SingularBatchProvider.Reset(ctx, sysCfg)
}
